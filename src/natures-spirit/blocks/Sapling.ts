...
world.beforeEvents.worldInitialize.subscribe(({ blockComponentRegistry }) => {
  blockComponentRegistry.registerCustomComponent("natures_spirit:sapling", {
    onRandomTick({ block, dimension }) {
      ...
      if (!block.permutation.getState("natures_spirit:age_bit" as any)) {
        block.setPermutation(
          block.permutation.withState("natures_spirit:age_bit" as any, true)
        );
      } else if (block.permutation.getState("natures_spirit:age_bit" as any)) {
        if (
          type == "redwood" &&
